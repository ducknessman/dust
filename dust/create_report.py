...
class CreateReport:
    ...
    def __init__(self):
        basic = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))
        self.path = f'{basic}/app/templates/report'
...
<script type="text/javascript">
    ...
    def create(self,data,name):
        body,js,foot = self.body(),self.js(data),self.foot()
        with open(self.path + '/' + name + '.html','w',encoding='utf-8') as f:
            info = body + js + foot
            f.write(str(info))
...
if __name__ == '__main__':
    ...
    "testName": "20201109132744897",
    "testAll": 7,
    "testFail": 1,
    "beginTime": "2020-11-09 13:27:44.917",
    "totalTime": "11ms",
    "testSkip": 2
    }
    name = '测试数据.html'
    CreateReport().create(data,name)