...
class HTMLTestRunner(Template_mixin):
    ...
    def __init__(self, start_time, stream=sys.stdout, verbosity=1, title=None, description=None):
        self.stream = stream                                            #需要读取的文件操作
    ...
    def _generate_report(self, result):
        ...
        for cid, (cls, cls_results) in enumerate(sortedResult):
            ...
            if ne > 0:
                liTestActive = '<li class="test displayed active has-leaf error" status="error" bdd="error" test-id="' + desc + '_' + str(
                    cid + 1) + '">'
...
class Suite:
    def __init__(self,success_count,failure_count,error_count,result):
...
def create_report(task_name,report_id,*args):
    info = Suite(*args)
    basic = os.path.abspath(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
    path = "{}/app/templates/report/{}_{}.html".format(basic,task_name,report_id)
    running_time = datetime.datetime.now()
    with open(path, 'wb') as fp:
        runner = HTMLTestRunner(
            stream=fp,
            title='This is MyTest',
            description='the test is show results and info of collection',
            start_time=running_time
        )
        runner.STYLESHEET_TMPL = '<linkrel="stylesheet" href="my_stylesheet.css" type="text/css">'
        runner.run(info)